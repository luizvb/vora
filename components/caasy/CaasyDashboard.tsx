...
export function CaasyDashboard({ initialRole, initialView }: { initialRole?: string; initialView?: string }) {
  ...
  if (!isSupabaseConfigured) {
    return <CenteredState title="Supabase is not configured" text="Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to enable the app." />;
  }
...
function RoleHome({ profile, reports, onOpen }: { profile: RoleProfile; reports: Report[]; onOpen: (view: ViewId) => void }) {
  ...
  return (
    <div className="mx-auto max-w-[980px]">
      <WelcomeBanner
        title="CaaSy coaching dashboard"
        text="A focused MVP: analyze real calls, run coaching sessions, and track only AI-generated insights saved to Supabase."
        cta="Coach my Call"
      ...
      <div className="mb-3 grid grid-cols-2 gap-3">
        <ActionCard
          title="Coach my Call"
          text="Paste a call transcript. The agent orchestration analyzes the conversation, signals, language, and next steps."
          action="Analyze a call"
          onClick={() => onOpen("coach-call")}
        />
        <ActionCard
          title="Coach Me"
          text="Paste a situation, question, or context. CaaSy returns practical coaching and saves the session."
...
function buildDynamicStats(reports: Report[]) {
  if (reports.length === 0) {
    return [
      { value: "0", label: "AI reports", detail: "No saved analysis yet", tone: "blue" as const, view: "coach-call" as const },
      { value: "-", label: "Average score", detail: "Run first analysis", tone: "blue" as const },
      { value: "0", label: "Action items", detail: "Generated by AI", tone: "warn" as const },
  ...
  return [
    { value: String(reports.length), label: "AI reports", detail: "Saved in database", tone: "blue" as const, view: "my-sessions" as const },
...
function EmptyDashboard({ onOpen }: { onOpen: (view: ViewId) => void }) {
  return (
    <Card className="mb-3">
      ...
      <h2 className="font-display text-2xl text-[#1A2530]">Your dashboard is empty because no session has been saved yet.</h2>
      <p className="mt-2 max-w-2xl text-sm leading-6 text-[#607080]">
        No mock data is shown. The dashboard updates after a call or coaching session runs through AI and is saved to Supabase.
      </p>
    ...
    <div className="mb-3 grid grid-cols-[1fr_320px] gap-3">
      <Card>
        <div className="mb-4 flex items-center justify-between">
          <CardTitle className="mb-0">Latest AI coaching insights</CardTitle>
      ...
      <Card>
        <CardTitle>Action plan from AI</CardTitle>
...
function ReportsView({ reports, compact = false }: { reports: Report[]; compact?: boolean }) {
  ...
  return (
    <div className={compact ? "" : "mx-auto max-w-[980px]"}>
      <Card>
        <CardTitle>{compact ? "Recent AI reports" : "Reports"}</CardTitle>
...
function CoachingView({ role }: { role: CaaSyRole }) {
  ...
  return (
    <div className="mx-auto grid max-w-[980px] grid-cols-[1fr_300px] gap-3">
      ...
      <Card>
        <CardTitle>Real AI flow</CardTitle>
        <div className="space-y-2">
          <InfoBlock title="Same orchestration" text="Supervisor, sales, coach, linguistics and analyst agents run through /api/analyze." />
          <InfoBlock title="Saved report" text="The finished coaching output is persisted and reflected on the dashboard." color="grey" />
        </div>
      </Card>
    </div>
  );
    ...
    <div className="mx-auto max-w-[980px]">
      <div className="mb-3 grid grid-cols-[1fr_310px] gap-3">
        <Card>
          ...
          {!isAnalyzing ? (
            <>
              <h1 className="font-display text-3xl text-[#1A2530]">Analyze a real call.</h1>
              <p className="mt-2 max-w-xl text-sm leading-6 text-[#607080]">Paste a transcript. CaaSy runs the agent orchestration, saves the report, and updates the dashboard with real insights.</p>
              <div className="mt-5">
        ...
        <Card>
          <CardTitle>Real MVP flow</CardTitle>
          <div className="space-y-2">
            <InfoBlock title="Transcript analysis" text="This is wired to the existing multi-agent analysis API." />
            <InfoBlock title="Report history" text="Completed reports are saved to Supabase and appear in your history." color="grey" />
          </div>
        </Card>
      </div>
    </div>
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden p-3 md:hidden">
      <div className="mb-3 rounded-xl bg-white p-3 text-sm text-[#607080] shadow-xs">
        <strong className="text-[#1A2530]">{profile.pill}</strong> · Simple functional MVP.
      </div>
      <div className="grid gap-2">
        <button type="button" onClick={() => router.push(dashboardPath(role, "coach-call"))} className="rounded-[11px] bg-[#1E4A6E] p-4 text-left text-white">
          ...
          <div className="mt-1 text-xs text-white/65">Analyze a real transcript.</div>
        </button>
        <button type="button" onClick={() => router.push(dashboardPath(role, "coach-me"))} className="rounded-[11px] border border-[#E4E9ED] bg-white p-4 text-left text-[#1A2530]">
          ...
          <div className="mt-1 text-xs text-[#607080]">Start a coaching session with the same AI.</div>
        </button>
      </div>
    </div>
  );
