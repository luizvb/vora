...
export default function DashboardDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#F2F5F7]">
      <header className="flex h-12 items-center justify-between border-b border-[#E4E9ED] bg-white px-5">
      ...
      <div className="mx-auto max-w-[1120px] space-y-3 p-4">
        <div className="grid grid-cols-[minmax(0,1fr)_252px] gap-3">
          <Card className="bg-linear-to-br from-[#1E4A6E] to-[#2B6CB0] text-white">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="mb-3 inline-flex rounded-full bg-white/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-[1px]">Coaching complete</div>
                <h1 className="font-display text-[34px] leading-tight">{report.mode === "coach_me" ? "CaaSy Personal Coaching Report" : "CaaSy Sales Call Coaching Report"}</h1>
                <p className="mt-2 max-w-xl text-sm leading-5 text-white/70">{report.actionPlan[0]?.description || "This report turns your coaching session into clear next steps."}</p>
              </div>
              <div className="rounded-[14px] border border-white/15 bg-white/10 p-4 text-center">
                <div className="font-display text-[52px] leading-none">{report.overallScore}</div>
          ...
          <Card>
            ...
            <Button onClick={() => router.push(`/dashboard?role=individual&view=${report.mode === "coach_me" ? "coach-me" : "coach-call"}`)} className="mb-2 h-10 w-full bg-[#1E4A6E] text-white">
              <ArrowLeft className="mr-2 size-4" /> New session
            </Button>
            <Button disabled variant="outline" className="h-10 w-full">
          ...
          <Card>
            ...
            <div className="space-y-3">
              {report.pros.map((pro, index) => (
                <div key={index} className="rounded-lg border border-[#1D8A5E]/15 bg-[#E6F5EE]/60 p-3">
                  ...
                  <p className="mt-2 text-sm leading-5 text-[#607080]">{pro.analysis}</p>
                </div>
              ))}
            </div>
          </Card>
          <Card>
            ...
            <div className="space-y-3">
              {report.cons.map((con, index) => (
                <div key={index} className="rounded-lg border border-[#3B8FD4]/15 bg-[#EBF4FF]/70 p-3">
                  ...
                  <p className="mt-2 text-sm leading-5 text-[#607080]">{con.analysis}</p>
                </div>
              ))}
            </div>
          </Card>
          ...
          <div className="grid grid-cols-3 gap-3">
            {report.actionPlan.map((action, index) => (
              <div key={action.title} className="relative rounded-lg border border-[#E4E9ED] bg-[#F5F9FF] p-4">
                <div className="mb-1 font-display text-3xl text-[#3B8FD4]/25">0{index + 1}</div>
